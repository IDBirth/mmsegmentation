...
class BilinearConvTranspose2d(nn.Module):
    ...
    ref: https://mlgiti.djicorp.com/plugins/gitiles/mlc/+/refs/heads/master/experiment/VIS/models/utils/upsample_add.py#25
    ...
    def __init__(self, channels, scale_factor=2, with_groups=True, freeze_weights=True):
        ...
        self.nncf_init_weights(freeze_weights)

    def nncf_init_weights(self, freeze_weights):
    ...
    def get_upsampling_weight(in_channels, out_channels, kernel_size):
        ...
        weight = np.zeros((out_channels, in_channels, kernel_size, kernel_size), dtype=np.float64)
...
class NearestConvTranspose2d(nn.Module):
    ...
    def __init__(self, channels, scale_factor=2, with_groups=False, freeze_weights=True):
        ...
        self.nncf_init_weights(freeze_weights, with_groups)

    def nncf_init_weights(self, freeze_weights, with_groups):
        for m in self.modules():
            if isinstance(m, nn.ConvTranspose2d):
                out_channels, in_channels, kh, kw = m.weight.size()
                m.weight.data.copy_(
